...
function createIssueLinkNode(value: string, href: string): MarkdownNode {
  return {
    type: "link",
    url: href,
...
function rewriteMarkdownTree(node: MarkdownNode) {
  ...
  if (node.type === "link" || node.type === "linkReference" || node.type === "inlineCode" || node.type === "code" || node.type === "definition" || node.type === "html") {
    return;
  }
